...
按键使用 EXTI 中断记录事件，主循环执行页面切换和阈值修改。消抖采用“按下锁定、稳定松开后解锁”的方式，保证一次按下只产生一次操作；温度阈值步进为 1.0 C，湿度阈值步进为 1.0%，光照 ADC 阈值步进为 100。
...
1. 将阈值写入内部 Flash，实现掉电保存。
2. 在 Python 上位机中增加历史数据曲线和数据导出。
