...
void SensorData_Send(EnvironmentData *data, Threshold *thr)
{
    ...
    uint16_t temperature_fan_on_x10_raw;
    uint16_t temperature_high_alarm_x10_raw;
    ...
    payload[6] = data->smoke_alarm;
    payload[7] = data->water_alarm;
    payload[8] = data->alarm;

    temperature_fan_on_x10_raw = (uint16_t)thr->fan_temp_on_x10;
    temperature_high_alarm_x10_raw = (uint16_t)thr->high_temp_alarm_x10;

    payload[9] = (uint8_t)(temperature_fan_on_x10_raw >> 8);
    payload[10] = (uint8_t)temperature_fan_on_x10_raw;

    payload[11] = (uint8_t)(temperature_high_alarm_x10_raw >> 8);
    payload[12] = (uint8_t)temperature_high_alarm_x10_raw;

    payload[13] = (uint8_t)(thr->high_humidity_alarm_x10 >> 8);
    payload[14] = (uint8_t)thr->high_humidity_alarm_x10;

    payload[15] = (uint8_t)(thr->light_on_threshold >> 8);
    payload[16] = (uint8_t)thr->light_on_threshold;

    UART_SendFrame(CMD_DATA, payload, 17);
}
...
{
    HAL_UARTEx_ReceiveToIdle_DMA(&huart2, rx_dma_byte, sizeof(rx_dma_byte));
...
uint8_t UART_ProcessCommand(Threshold *threshold)
{
    ...
    if (rx_cmd == CMD_SET_THRESHOLD)
    {
        ...
        switch (parameter_id)
        {
        case PARAM_TEMP_FAN_ON_MAX:
            if ((temperature_value >= -400) && (temperature_value < threshold->high_temp_alarm_x10 - 30))
            {
                threshold->fan_temp_on_x10 = temperature_value;
                updated = 1;
            }
            else
            {
        ...
        case PARAM_TEMP_HIGH_ALARM_MAX:
            if ((temperature_value > threshold->fan_temp_on_x10 + 30) && (temperature_value <= 850))
            {
                threshold->high_temp_alarm_x10 = temperature_value;
                updated = 1;
            }
            else
            {
        ...
        case PARAM_HUMI_HIGH_ALARM_MAX:
            if (value <= 1000)
            {
                threshold->high_humidity_alarm_x10 = value;
                updated = 1;
            }
            else
            {
        ...
        case PARAM_LIGHT_DARK:
            if (value <= 4095)
            {
                threshold->light_on_threshold = value;
                updated = 1;
            }
            else
            {
                           ...
                           uint8_t length)
{
    if (tx_busy == 0)
    {
        ...
        HAL_UART_Transmit_DMA(&huart2, tx_dma_buffer, index);
    }
}
