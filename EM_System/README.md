...
上位机通信使用 USART1：`PA9(TX)`、`PA10(RX)`。

CI-03T 语音模块使用 USART3 普通中断接收，当前引脚为 `PD8(TX)`、`PD9(RX)`，模块与 STM32 的收发线交叉连接，参数为 `115200、8N1、无校验、无流控`。语音模块的 `UART1_TX/UART1_RX` 是模块内部编号，不等于 STM32 的 USART1。
...
USART3 也可接 USB-TTL 串口助手进行控制帧测试。发送模式选择 HEX，发送 `AA 55 03 55 AA` 可开启风扇。USB-TTL 的 TX 接 `PD9`，RX 接 `PD8`，并与开发板共地。

## 自动控制规则

- 自动模式下，温度达到风扇开启阈值时启动风扇，降至关闭阈值后关闭；开启和关闭阈值之间保持当前状态。
- 烟雾或高温报警时风扇强制开启，安全保护优先于手动关闭命令。
- 检测到水浸时启动水泵，积水消失后关闭水泵。
- 光照不足时打开照明，光照恢复后关闭照明。
- 温度或湿度超过报警阈值时启动蜂鸣器。

手动模式下，风扇、照明和水泵由对应的手动状态控制；烟雾、高温和水浸保护仍然有效。
